...
class ProxyProvider(CredentialsProvider):
    ...
    def authenticate(self) -> tuple[str, timedelta] | None:
        ...
        remote_user = self.headers.get(settings.REMOTE_USER_HEADER, None)  # pyright:ignore -- headers should be set in /auth/token handler, i.e. before we reach here

        if not remote_user:
            self._logger.debug(f"Required header {settings.REMOTE_USER_HEADER} not found")
